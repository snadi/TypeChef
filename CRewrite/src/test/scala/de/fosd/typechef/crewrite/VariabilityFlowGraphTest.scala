...
class VariabilityFlowGraphTest extends FunSuite with TestHelper with ShouldMatchers with VariabilityFlowImpl {
  ...
  private def parsePrintGetSucc(code: String, pro: p.MultiParser[AST]) = {
    val ast = parse(code, cp(pro))
    println("AST: " + ast.get)
    succvar(ast.get.asInstanceOf[One[AST]].value)
  }
